...
-------------------------------------------------------------------------------
function mob_inventory_2.allow_move(inv, from_list, from_index, to_list, to_index, count, player)
...
-------------------------------------------------------------------------------
function mob_inventory_2.allow_put(inv, listname, index, stack, player)
...
-------------------------------------------------------------------------------
function mob_inventory_2.allow_take(inv, listname, index, stack, player)
...
--! @param player doing changes
-------------------------------------------------------------------------------
function mob_inventory_2.on_move(inv, from_list, from_index, to_list, to_index, count, player)
	dbg_mobf.trader_inv_lvl1("MOBF: inv\"" .. tostring(inv) .. "\" moving " 
		.. count .. " items from: " .. from_list .. ":" .. from_index .. " to: "
		.. to_list .. ":" .. to_index)

	if from_list == "goods_2" and
		...
		if( elements > count ) then
			...
			inv.set_stack(inv,"goods_2",from_index, 
				goodname.." "..tostring( elements - count ))
			-- update the real amount of items in the slot now
			elements = count
		end
		
		local entity = mob_inventory_2.get_entity(inv)
		...
		mob_inventory_2.fill_prices(entity,inv,goodname,count)
		mob_inventory_2.update_takeaway(inv)
	end
end
...
--! @param player doing changes
-------------------------------------------------------------------------------
function mob_inventory_2.on_put(inv, listname, index, stack, player)
	if listname == "pay" then
...
--! @param player doing changes
-------------------------------------------------------------------------------
function mob_inventory_2.on_take(inv, listname, index, stack, player)
	if listname == "takeaway" then
		...
		if not mob_inventory_2.check_pay(inv,true) then
	...
	if listname == "pay" then
		if mob_inventory_2.check_pay(inv,false) then
...
--! @param inv to update
-------------------------------------------------------------------------------
function mob_inventory_2.update_takeaway(inv)
	if mob_inventory_2.check_pay(inv,false) then
		local selection = inv.get_stack(inv,"selection", 1)
...
--! @return true/false
-------------------------------------------------------------------------------
function mob_inventory_2.check_pay(inv,paynow)
	local now_at_pay = inv.get_stack(inv,"pay",1)
	local count = now_at_pay.get_count(now_at_pay)
	local name  = now_at_pay.get_name(now_at_pay)
...
function mob_inventory_2.init_trader_inventory_2(entity)
	...
	local trader_inventory_2 = 
	minetest.create_detached_inventory(unique_entity_id,
	{
		allow_move 	= mob_inventory_2.allow_move,
		allow_put 	= mob_inventory_2.allow_put,
		allow_take 	= mob_inventory_2.allow_take,
	
		on_move 	= mob_inventory_2.on_move,
		on_put 		= mob_inventory_2.on_put,
		on_take 	= mob_inventory_2.on_take,
	})
	
	trader_inventory_2.set_size(trader_inventory_2,"goods_2",16)
	trader_inventory_2.set_size(trader_inventory_2,"takeaway",1)
	trader_inventory_2.set_size(trader_inventory_2,"selection",1)
	trader_inventory_2.set_size(trader_inventory_2,"price_1",1)
	trader_inventory_2.set_size(trader_inventory_2,"price_2",1)
	trader_inventory_2.set_size(trader_inventory_2,"pay",1)
	
	mob_inventory_2.add_goods_2(entity,trader_inventory_2)
	
	--register to trader inventories
	table.insert(mob_inventory_2.trader_inventories, {
										identifier 	= unique_entity_id,
										inv_ref 	= trader_inventory_2,
										ent_ref 	= entity,
										})
	dbg_mobf.trader_inv_lvl3(
		"MOBF: registering identifier: " .. unique_entity_id 
		.. " invref \"" .. tostring(trader_inventory_2) .. "\"  for entity \"" 
		.. tostring(entity) .. "\"" )

	local trader_formspec = "size[8,10;]" ..
			"label[2,0;Trader " .. tradername .. " Inventory]" .. 
			"label[0,1;Selling:]" ..
			"list[detached:" .. unique_entity_id .. ";goods_2;0,1.5;8,2;]" ..
	...
	if mob_inventory_2.register_formspec("formspec_" 
...
-------------------------------------------------------------------------------
function mob_inventory_2.config_check(entity)
	if entity.data.trader_inventory_2 ~= nil then
		return true
	end
...
-------------------------------------------------------------------------------
function mob_inventory_2.register_formspec(name,formspec)

	if mob_inventory_2.formspecs[name] == nil then
		mob_inventory_2.formspecs[name] = formspec
		return true
	end
...
-------------------------------------------------------------------------------
function mob_inventory_2.trader_callback_2(entity,player)
	local unique_entity_id = string.gsub(tostring(entity),"table: ","")
	--local unique_entity_id = "testinv"
	local playername = player.get_player_name(player)
	
	if mob_inventory_2.formspecs["formspec_" .. unique_entity_id] ~= nil then
		...
		if minetest.show_formspec(playername,
					...
					mob_inventory_2.formspecs["formspec_" .. unique_entity_id]) 
					== false then
...
-------------------------------------------------------------------------------
function mob_inventory_2.get_entity(inv)
	...
	if location.type == "detached" then
		for i=1,#mob_inventory_2.trader_inventories,1 do
			dbg_mobf.trader_inv_lvl3("MOBF: comparing \"" 
				.. location.name .. "\" to \"" 
				.. mob_inventory_2.trader_inventories[i].identifier .. "\"")
			if mob_inventory_2.trader_inventories[i].identifier 
				== location.name then
				return mob_inventory_2.trader_inventories[i].ent_ref
			end
		end
	end
...
-------------------------------------------------------------------------------
function mob_inventory_2.fill_prices(entity,inventory,goodname,count)

	--get price info from entity
	local good = nil
	
	for i=1,#entity.data.trader_inventory_2.goods_2,1 do
		local stackstring = goodname .." " .. count
		dbg_mobf.trader_inv_lvl3("MOBF: comparing \"" .. stackstring .. "\"" ..
			" to \"" .. entity.data.trader_inventory_2.goods_2[i][1] .. "\"")
		if entity.data.trader_inventory_2.goods_2[i][1] == stackstring then
			good = entity.data.trader_inventory_2.goods_2[i]
		end
	end
...
-------------------------------------------------------------------------------
-- name: add_goods_2(entity,trader_inventory_2)
--
--! @brief fill inventory with mobs goods_2
--! @memberof mob_inventory_2
--! @private
--
--! @param entity to look for prices
--! @param trader_inventory_2 to put goods_2
-------------------------------------------------------------------------------
function mob_inventory_2.add_goods_2(entity,trader_inventory_2)

	local goods_2_to_add = nil

	if entity.data.trader_inventory_2.goodlist ~= nil then
		local total_sum_chances = 0
		
		for i=1,#entity.data.trader_inventory_2.goodlist,1 do
			total_sum_chances = total_sum_chances + 
				entity.data.trader_inventory_2.goodlist[i].chance
		end
		...
		for j=1,#entity.data.trader_inventory_2.goodlist,1 do
		
			runvalue = runvalue + 
				entity.data.trader_inventory_2.goodlist[j].chance
		...
		goods_2_to_add = 
			entity.data.trader_inventory_2.goodlist[selected_index].goods_2
	else
		goods_2_to_add = entity.data.trader_inventory_2.goods_2
	end

	dbg_mobf.trader_inv_lvl3("MOBF: adding " 
		.. #goods_2_to_add 
		.. " goods_2 for trader")
	for i=1,#goods_2_to_add,1 do
		dbg_mobf.trader_inv_lvl3("MOBF:\tadding " .. 
			goods_2_to_add[i][1])
		trader_inventory_2.set_stack(trader_inventory_2,"goods_2", i,
									goods_2_to_add[i][1])
	end
